...
package org.chipsalliance.diplomacy.amba.ahb
...
import org.chipsalliance.diplomacy.amba._
import org.chipsalliance.diplomacy.amba.ahb.{AHBImpSlave}
import org.chipsalliance.diplomacy.TransferSizes
import org.chipsalliance.diplomacy.tilelink.{TLImp, TLMasterPortParameters, TLMessages, TLMasterParameters, TLMasterToSlaveTransferSizes}
import org.chipsalliance.diplomacy.util.{BundleMap, MaskGen, DataToAugmentedData}
import org.chipsalliance.cde.config.Parameters

// Add this "Unsafe" variant that does not forward the AMBAProtField. This is not necessary for our tapeout needs because we are
// not worried about cache coherence or any fancy memory system stuff.
case class AHBToTLUnsafeNode()(implicit valName: ValName) extends MixedAdapterNode(AHBImpSlave, TLImp)(
  dFn = { case mp =>
    TLMasterPortParameters.v2(
      ...
      requestFields = mp.requestFields,
...
class AHBToTLUnsafe()(implicit p: Parameters) extends LazyModule
{
...
object AHBToTLUnsafe
{
  def apply()(implicit p: Parameters) =
  {
    val ahb2tl = LazyModule(new AHBToTLUnsafe)
    ahb2tl.node
  }
}
