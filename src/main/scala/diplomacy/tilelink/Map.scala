...
//       def forward(x: UInt) =
//         convert.map { case (i, o) => Mux(i.contains(x), UInt(o.base) | (x & UInt(o.mask)), 0.U) }.reduce(_ | _)
//       def backward(x: UInt) =
//         convert.map { case (i, o) => Mux(o.contains(x), UInt(i.base) | (x & UInt(i.mask)), 0.U) }.reduce(_ | _)
